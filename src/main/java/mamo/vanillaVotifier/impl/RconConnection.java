...
public class RconConnection implements Rcon {
	private final Config.RconConfig rconConfig;

	private Socket socket;
	private int requestId;

	public RconConnection(Config.RconConfig rconConfig) {
		if (rconConfig == null) {
	...
	@Override
	public Config.RconConfig getRconConfig() {
		return rconConfig;
	}
	...
	public synchronized boolean isConnected() {
		if (socket != null) {
			try {
				sendRequest(new VanillaVotifierPacket(requestId, Packet.Type.COMMAND, null));
				return true;
			} catch (Exception e) {
	...
	public Packet logIn() throws UnsupportedEncodingException, IOException {
		return sendRequest(new VanillaVotifierPacket(requestId, Packet.Type.LOG_IN, rconConfig.getPassword()));
	}
	...
	public synchronized Packet sendRequest(Packet request) throws UnsupportedEncodingException, IOException {
		...
		Packet.Type responseType = Packet.Type.fromInt(responseBuffer.getInt());
